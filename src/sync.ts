  ...
  const updates: string[] = await [appSkuBatchData].reduce(
    async (accumPromise: Promise<string[]>, inventorySkuBatchData: SkuBatchData[]) => {
      ...
      if (appSkuBatchData.length != inventorySkuBatchData.length) {
        ...
        let appDataIds = new Set(appSkuBatchData.map(item => item.skuBatchId));
        let missingIds = inventorySkuBatchData.filter(item => !appDataIds.has(item.skuBatchId)).map(item => item.skuBatchId);

        if (missingIds.length > 0) {
          logger.log(`Missing IDs from app data that exist in the inventory data: ${missingIds.join(', ')}`);
        }
      }
...
const inventoryApiUrl = 'https://local-inventory.nabis.dev/v1';
interface CreateInventoryRecordPayload {
  skuBatchId: string;
  skuId: string;
  wmsId: string;
...
interface UpdateInventoryRecordPayload {
  skuBatchId: string;
  skuId: string;
  wmsId: string;
  quantityPerUnitOfMeasure: number;
...
interface CreateInventoryAggregatePayload {
  skuBatchId: string;
  skuId: string;
  wmsId: string;
  quantityPerUnitOfMeasure: number;
...
interface UpdateInventoryAggregatePayload {
  skuBatchId: string;
  skuId: string;
  wmsId: string;
  quantityPerUnitOfMeasure: number;
...
const createInventoryRecord = async (payload: CreateInventoryRecordPayload): Promise<void> => {
  try {
    await axios.post(`${inventoryApiUrl}/inventory`, payload);
  } catch (error) {
...
const updateInventoryRecord = async (payload: UpdateInventoryRecordPayload): Promise<void> => {
  try {
    await axios.put(`${inventoryApiUrl}/inventory`, payload);
  } catch (error) {
...
const createInventoryAggregate = async (payload: CreateInventoryAggregatePayload): Promise<void> => {
  try {
    await axios.post(`${inventoryApiUrl}/inventory-aggregate`, payload);
  } catch (error) {
    console.error('Error creating or updating inventory aggregate:', error);
    throw error;
  }
};

const updateInventoryAggregate = async (payload: UpdateInventoryAggregatePayload): Promise<void> => {
  try {
    await axios.put(`${inventoryApiUrl}/inventory-aggregate`, payload);
  } catch (error) {
    console.error('Error updating inventory aggregate:', error);
    throw error;
  }
};
*/