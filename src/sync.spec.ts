...
describe('sync', () => {
  ...
  describe('.makeUpdates', () => {
    it('should create a list of string sql updates based on a update delta', async () => {
      const update : skuBatchUpdate = {
        skuBatchId: 'sku-batch-id-1',
        updates: [ { field: 'isArchived', newValue: true } ]
      }
      const sql : string[] = makeUpdates(update);
      expect(sql).toStrictEqual([
        "update inventory set is_archived = true where sku_batch_id = 'sku-batch-id-1'",
        "update inventory_aggregate set is_archived = true where sku_batch_id = 'sku-batch-id-1'"
      ]);
    })
  })
});
